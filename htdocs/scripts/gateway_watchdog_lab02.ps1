...
# Configuração principal (Movel-SafeGreen presumido em lab02)
$InstanceSlug = 'lab02'
...
$StartCommand = Join-Path $RepoRoot 'services/whatsapp-web-gateway/start-gateway-lab02.bat'
$LogPath = Join-Path $RepoRoot 'storage/logs/gateway_watchdog_lab02.log'
$StatePath = Join-Path $RepoRoot 'storage/logs/gateway_watchdog_lab02.state.json'
...
function Load-State {
    if (Test-Path $StatePath) {
        try {
            ...
            Write-Log "Estado corrompido, reiniciando contadores"
        }
    }
...
if (!(Test-Path $StartCommand)) {
    Write-Log "Start command nao encontrado: $StartCommand"
    exit 1
}
